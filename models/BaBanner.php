 ...
 * @property string $ba_begin
 * @property string $ba_expire
...
class BaBanner extends BaModelAbstract
{
	...
	{
		if (!empty($this->_data['bac_id']))
			return BaClient::getById($this->_data['bac_id']);
		return NULL;
	}

	public function setClient($column, $value)
	{
		if ($value instanceof BaClient)
			$this->_data['bac_id'] = $value->bac_id;
		if (is_array($value))
			$this->_data['bac_id'] = $value['bac_id'];
		if (is_integer($value))
			$this->_data['bac_id'] = $value;
		return NULL;
	}
	...
	public function getTrackClics($column)
	{
		...
		elseif ($this->_data['ba_track_clicks'] == -1)
		{
			if (empty($this->client))
			{
				...
				if ($cfg["plugin"]['banners']['track_clicks'] == 1)
					$trackClics = true;
			}else
			{
	...
	public function getTrackImpressions($column)
	{
		...
		elseif ($this->_data['ba_track_impressions'] == -1)
		{
			...
			{
				// Если не установлен клиент, берем настройки по-умолчанию
				if ($cfg["plugin"]['banners']['track_impressions'] == 1)
					$track = true;
			}else
			{
	...
	public function save($data = null)
	{
		...
		$data['ba_created'] = $sys['now'];
		$data['ba_created_by'] = $usr['id'];

		if (!$data['ba_id'])
		{
			// Добавить новую запись
			$data['ba_updated'] = $sys['now'];
		...
		if (!empty($this->currFile) && isset($data['ba_file']) && $this->currFile != $data['ba_file'] &&
			file_exists($this->currFile))
			unlink($this->currFile);
	...
	public function impress()
	{
		...
		if ($this->trackImpressions)
		{
			...
			if ($count)
			{
				...
				$db->update($db_ba_tracks, $data, "track_type=1 AND ba_id={$this->_data['ba_id']} AND track_date='{$trackDate}'");
			}
			else
			{
				// insert new count
	...
	public function click()
	{
		...
		if ($this->trackClics)
		{
			...
			if ($count)
			{
				...
				$db->update($db_ba_tracks, $data, "track_type=2 AND ba_id={$this->_data['ba_id']} AND track_date='{$trackDate}'");
			}
			else
			{
				// insert new count
