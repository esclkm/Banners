...
//        echo "$className -> $pk : No Cache<br />";
		...
		if ($res)
			self::$_stCache[$className][$pk] = $res[0];
			...
			foreach ($conditions as $condition)
			{
				...
				if (is_array($condition))
				{
					...
					if (empty($condition[2]))
						$condition[2] = '=';
					if (empty($condition[3]) || $condition[3] != 'OR')
						$condition[3] = 'AND';
				...
				else
				{
					...
					if ($column && $operator)
					{
						...
						if ((intval($value) == $value) && (strval(intval($value)) == $value))
							$value = intval($value);
						$params[$column] = $value;
					}else
					{
...
//            if (is_string($str)) $ret[$key] = $db->quote($str);
			if (!(strval(intval($ret[$key])) == $ret[$key]))
				$ret[$key] = $db->quote($str);
		}
		return $ret;
	}
...
//            var_dump($btArray[$btIndex]);
			...
			else
			{
				...
				{
					preg_match('/([a-zA-Z\_]+)::/', $callerLine, $matches);
				}
				else
				{
					preg_match('/([a-zA-Z\_]+)::'.$functionName.'/', $callerLine, $matches);
				}
				$btIndex++;
				if (isset($matches[1]))
				{
